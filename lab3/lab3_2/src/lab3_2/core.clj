...
(defn my-filter-no-lazy [pred coll] (reduce
                             (fn [acc elem] 
                               (if (pred elem) (concat acc (list elem)) acc))
                             (list) coll))
...
(defn -main
  ...
  (let [n 400000, thread-number 2, thread-number-double (* 2 thread-number), thread-number-4 (* 4 thread-number)
